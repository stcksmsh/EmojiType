...
(async function init() {
  ...
  try {
    var r = await fetch(chrome.runtime.getURL("data/default-dictionary.json"));
    ...
    function (request, _sender, sendResponse) {
      if (request.action === "get") {
        ...
        if (request.key !== undefined && request.value !== undefined) {
          ...
          whitelistOn =
            request.whitelist.state === true ||
            request.whitelist.state === "on";
          ...
          blacklistOn =
            request.blacklist.state === true ||
            request.blacklist.state === "on";
          chrome.storage.local.set({
